...
def oesolvate(solute, density=1.0, padding_distance=10.0,
              ...
              neutralize_solute=True, verbose=False, return_components=False, 
    ...
    random_seed: integer
        If not None the random_seed will be passed as the argument to 'seed' in 
        the Packmol configuration file
    ...
    if neutralize_solute:
        ...
        if n_ions >= 1:
            ...
            for i in range(0, len(ions_smiles)):
                with oechem.oemolostream(ions_smiles_pdbs[i]) as ofs:
    ...
    if salt_concentration > 0.0*unit.millimolar:
        ...
        if n_salt >= 1:
            ...
            for i in range(0, len(salt_smiles)):
                with oechem.oemolostream(salt_pdbs[i]) as ofs:
    ...
    if geometry in ['box', 'rectangle']:
        ...
        # Set box vectors also on the solvent
        solvent_comp.SetData(oechem.OEGetTag('box_vectors'), box_vectors)
